...
//  NMAPScanner
//
//  Ported verbatim from AIStudio (kochj23/AIStudio) — the pure, network-free model
//  discovery + load-balancer core. Only the backend enum type is adapted to
//  NMAPScanner's existing `AIBackend`.
//
...
struct DiscoveredModel: Identifiable, Hashable, Sendable {
    ...
    init(modelName: String, displayName: String? = nil, backend: AIBackend, endpoint: String) {
