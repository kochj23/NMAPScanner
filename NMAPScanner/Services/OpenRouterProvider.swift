...
//  NMAPScanner
//
//  Ported from AIStudio (kochj23/AIStudio). OpenRouter frontier-model access + the
//  deterministic (network-free) pieces of the OpenAI-compatible request path and
//  the automatic-failover selection logic. Adapted to NMAPScanner's existing
//  `AIBackend` enum and `AIBackendError`; NMAPScanner's generate path has no chat
//  history, so the request builder takes prompt + system prompt only.
//
...
enum OpenRouterProvider {
    ...
    static let referer = "https://github.com/kochj23/NMAPScanner"
    static let title = "NMAPScanner"

    /// macOS Keychain service used to store the OpenRouter API key.
    static let keychainService = "com.jkoch.nmapscanner.openrouter"
...
enum OpenAICompatibleRequest {
    /// Map a prompt + optional system prompt into the OpenAI `messages` array shape.
...
enum FailoverPlanner {
    ...
    static let defaultChain: [AIBackend] = [.ollama, .mlx, .openRouter]

    /// The ordered subset of `chain` that is currently available.
    static func orderedHealthy(
        chain: [AIBackend],
        availability: [AIBackend: Bool]
    ) -> [AIBackend] {
