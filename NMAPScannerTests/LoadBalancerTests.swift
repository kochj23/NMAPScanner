...
//  composition, and the load-balancer selection policies. Adapted from
//  AIStudio's LoadBalancerTests to NMAPScanner's `AIBackend` enum.
//
...
final class LoadBalancerTests: XCTestCase {
    ...
    func testParseOllamaTagsMapsModels() {
        ...
        XCTAssertEqual(models[0].id, "\(AIBackend.ollama.rawValue)|mistral:latest")
    ...
    // MARK: - New backend types

    func testNewBackendTypes() {
        XCTAssertEqual(AIBackend.novaGateway.rawValue, "Nova Gateway")
        XCTAssertEqual(AIBackend.openRouter.rawValue, "OpenRouter")
        XCTAssertFalse(AIBackend.novaGateway.icon.isEmpty)
        XCTAssertFalse(AIBackend.openRouter.icon.isEmpty)
        XCTAssertTrue(AIBackend.allCases.contains(.openRouter))
        XCTAssertTrue(AIBackend.allCases.contains(.novaGateway))
    }

    // MARK: - Failover planner (pure)

    func testFailoverOrderedHealthy() {
        let availability: [AIBackend: Bool] = [.ollama: false, .mlx: true, .openRouter: true]
        let healthy = FailoverPlanner.orderedHealthy(chain: FailoverPlanner.defaultChain, availability: availability)
        XCTAssertEqual(healthy, [.mlx, .openRouter])
        XCTAssertEqual(FailoverPlanner.firstHealthy(chain: FailoverPlanner.defaultChain, availability: availability), .mlx)
    }
}
